$(function() {
    ...
    // hover变换图片
    $('.gl-item').find('.sider img').each(function() {
        $(this).hover(function() {
            $(this).parents('.gl-item').find('.p-img img').attr('src', $(this).attr('src'));
        })
    })
    ...
    // 主体图片
    // 将图片保存到客户端存储
    $('.gl-item').on('click', function() {
        $(this).find('.sider img').each(function(arr) {
            localStorage.setItem(arr, $(this).attr("src"));
        })
